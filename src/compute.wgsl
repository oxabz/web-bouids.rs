...
fn step([[builtin(global_invocation_id)]] global_invocation_id: vec3<u32>){
    ...
    loop {
        ...
        let dist = distance(oPos,oVel);

        if(dist < params.separationReach){
            sepSum = sepSum + normalize(vPos - oPos) / (distance(oPos, vPos)+1.0);
    ...
    if(sepCount>0u){
        sepSum = sepSum / f32(sepCount);
        vVel = vVel + sepSum * params.separationScale * params.deltaT;
    }
    if(aliCount>0u){
        aliSum = aliSum / f32(aliCount);
        vVel = vVel + aliSum * params.alignementScale * params.deltaT;
    }
    if(cohCount>0u){
        cohSum = cohSum / f32(cohCount);
        vVel = vVel + (- vPos + cohSum) * params.cohesionScale * params.deltaT;
    }
